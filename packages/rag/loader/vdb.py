...
class VectorDB:
  ...
  def setup(self, collection):
    ...
    if not collection in ls:
      ...
      schema.add_field(field_name="s3ImageKey", datatype=DataType.VARCHAR, max_length=DIMENSION_TEXT)
  ...
  def insert(self, text, s3ImageKey=""):
    vec = self.embed(text)
    return self.client.insert(self.collection, {"text":text, "embeddings": vec, "s3ImageKey": s3ImageKey})
  ...
  def vector_search(self, inp, limit=LIMIT):
    ...
    if len(cur[0]) > 0:
      for item in cur[0]:
        ...
        s3ImageKey = item.get("entity", {}).get("s3ImageKey", "")
        
        res.append((dist, text, s3ImageKey))
    return res
